...
function analyzeAST(ast, code) {
    ...
    function walk(node, parent) {
        ...
        for (const key in node) {
            if (['leadingComments', 'trailingComments', 'innerComments', 'loc', 'start', 'end'].includes(key)) continue;
...
function analyzeFile(filePath) {
    ...
    if (!fs.existsSync(docPath)) {
        return { status: 'missing_doc', path: filePath, docPath: docPath, methodCount: methods.length };
    }
    ...
    if (missingInDoc.length > 0) {
        return { status: 'inconsistent', path: filePath, docPath: docPath, missingInDoc };
    }
