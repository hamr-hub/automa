<template>
  <div class="mb-6 flex flex-wrap items-center md:space-x-4">
    ...
    <div class="workflow-sort ml-4 flex w-5/12 items-center md:ml-0 md:w-auto">
      ...
      >
        <option
v-for="sort in sortsList" :key="sort.id"
:value="sort.id"
>
          {{ sort.name }}
        </option>
    ...
    <ui-popover class="mt-4 md:mt-0">
      <template #trigger>
        <ui-button>
          <v-remixicon
name="riFilter2Line" class="mr-2 -ml-1"
/>
      ...
      <div class="w-48">
        ...
        >
          <option
v-for="date in filterByDate" :key="date.id"
:value="date.id"
>
            {{ date.name }}
          </option>
    ...
    <ui-button
class="ml-4 mt-4 md:ml-0 md:mt-0" @click="$emit('clear')"
>
      <v-remixicon
