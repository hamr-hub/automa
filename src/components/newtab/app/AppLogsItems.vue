<template>
  <div class="logs-list overflow-auto pb-4 pt-1">
    ...
    >
      <ui-popover
padding="" @click="filtersBuilder.workflowQuery = ''"
>
        <template #trigger>
          <ui-button>
            <span
class="text-overflow text-left" style="max-width: 160px"
>
              {{ activeWorkflowName }}
            </span>
            <v-remixicon
name="riArrowDropDownLine" class="-mr-1 ml-2"
/>
          </ui-button>
        </template>
        <div class="w-64">
    ...
    <div
v-if="logs" style="min-height: 320px"
>
      <shared-logs-table
        :logs="logs"
        :modal="true"
...
>
          <option
v-for="num in [10, 15, 25, 50, 100]" :key="num"
:value="num"
>
            {{ num }}
          </option>
    ...
    >
      ...
      <ui-button
variant="danger" @click="deleteSelectedLogs"
>
        {{ t('log.deleteSelected') }} ({{ selectedLogs.length }})
      </ui-button>
    </ui-card>
    <ui-modal
v-model="exportDataModal.show" content-class="max-w-2xl"
>
      <template #header>
