  ...
  <div class="space-y-6">
    <div v-if="!mfaData">
      <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">
        {{ t('auth.mfaDescription', '开启两步验证(2FA)可以为您的账号提供更高的安全性。即使密码泄露，黑客也无法登录您的账号。') }}
      </p>
    ...
    <div v-else class="space-y-6">
      <div class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <p class="mb-4 text-xs text-gray-500">
          {{ t('auth.scanQrCodeDesc', '请使用 Google Authenticator 或其他认证应用扫描下方二维码。') }}
        </p>
        ...
        <div class="mt-4 text-center">
          <p class="text-xs text-gray-500">{{ t('auth.cantScan', '无法扫描？请手动输入密钥：') }}</p>
          <code class="mt-1 block select-all rounded bg-gray-200 px-2 py-1 text-sm font-mono dark:bg-gray-700">
            {{ mfaData.totp.secret }}
          </code>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="flex gap-2">
          <ui-input
            ...
            maxlength="6"
          />
          <ui-button variant="accent" :loading="verifying" :disabled="verifyCode.length !== 6" @click="verify">
            {{ t('auth.verify', '验证并启用') }}
          </ui-button>
        </div>
