...
async function runWorkflow(workflowPath) {
  ...
  const getNextNode = (currentId, outputHandle = 'output-1') => {
    const edge = edges.find(
      (e) => e.source === currentId && e.sourceHandle.endsWith(outputHandle)
    );
    if (!edge) return null;
  ...
  while (currentNode) {
    console.log(
      `Executing [${currentNode.label}]: ${currentNode.data.description || ''}`
    );
    ...
    try {
      switch (currentNode.label) {
        ...
        case 'attribute-value': {
          ...
          const attr = el
            ? await el.getAttribute(currentNode.data.attributeName)
            : '';
        ...
        case 'while-loop':
          ...
          const condition =
            currentNode.data.conditions[0].conditions[0].items[0];
          if (condition.data.selector) {
            const exists = await page.$(condition.data.selector);
            console.log(
              `   -> Condition (Selector ${condition.data.selector} exists): ${!!exists}`
            );
        ...
        case 'loop-elements':
          ...
          console.log(
            `   -> Found ${elements.length} elements matching ${currentNode.data.elementSelector}`
          );
          ...
          console.log(
            '   -> Entering loop body (Testing extraction on first element)...'
          );
          ...
          while (bodyNode) {
            console.log(
              `      [Inside Loop] Executing [${bodyNode.label}]: ${bodyNode.data.description || ''}`
            );
            ...
            try {
              if (bodyNode.label === 'get-text') {
                ...
                const text = await page.$eval(
                  currentNode.data.elementSelector,
                  (el, sel) => {
                ...
                console.log(
                  `      -> Extracted [${bodyNode.data.dataColumn}]: ${text.substring(0, 50)}...`
                );
                if (!state.currentItem) state.currentItem = {};
                ...
                console.log(
                  `      -> Extracted Attr [${bodyNode.data.dataColumn}]: ${attr.substring(0, 50)}...`
                );
                if (!state.currentItem) state.currentItem = {};
        ...
        default:
          console.log(
            `   -> Unknown block type ${currentNode.label}, skipping`
          );
      }
    } catch (e) {
