...
const supabaseConfig = {
  ...
  supabaseAnonKey:
    process.env.SUPABASE_ANON_KEY || secrets.supabaseAnonKey || '',
};
...
async function testSupabaseConnection() {
  ...
  if (!supabaseConfig.supabaseUrl || !supabaseConfig.supabaseAnonKey) {
    console.warn(
      '[!] Missing Supabase Credentials. Connection test is expected to fail or skip.'
    );
  ...
  if (SupabaseClient.initialized) {
    ...
    try {
      ...
      console.log(
        `    -> Query successful. Found ${workflows.length} workflows.`
      );
    } catch (e) {
    ...
    console.error(
      '    -> Initialization failed. (SupabaseClient.initialized is false)'
    );
    console.log(
      '       Possible reasons: Invalid URL/Key, Network issues, or Supabase instance is down.'
    );
  }
}
