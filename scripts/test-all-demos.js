...
async function validateWorkflow(workflowPath) {
  ...
  try {
    ...
    const orphanNodes = nodes.filter(n => 
      n.label !== 'trigger' && !connectedNodes.has(n.id)
    );
    ...
    const getNextNode = (currentId, outputHandle = 'output-1') => {
      const edge = edges.find(e => 
        e.source === currentId && 
        (e.sourceHandle.includes(outputHandle) || e.sourceHandle.endsWith(outputHandle))
      );
      if (!edge) return null;
    ...
    while (currentNode && stepCount < maxSteps) {
      ...
      console.log(`   [${stepCount}] ${currentNode.label}: ${desc.substring(0, 40)}${desc.length > 40 ? '...' : ''}`);
...
async function runAllTests() {
  ...
  allResults.forEach((result, index) => {
    ...
    console.log(`   节点: ${result.stats.nodes}, 边: ${result.stats.edges}, 执行步骤: ${result.stats.executedSteps}`);
