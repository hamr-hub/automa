...
const workflow = generator.generateWorkflow(mockAiOutput, "Scrape Amazon laptops", "https://www.amazon.com/s?k=laptop");

// Save to file
fs.writeFileSync('amazon_workflow_ai_gen.json', JSON.stringify(workflow, null, 2));
...
const trace = (nodeId, indent = "") => {
    ...
    console.log(`${indent} [${node.label}] ${node.data.description || ''} (${node.id})`);
...
if (paginationNode) {
    // Check conditions
    console.log("Pagination Conditions:", JSON.stringify(paginationNode.data.conditions, null, 2));
}
...
if (paginationNode && exportNode) {
    const connection = edges.find(e => e.source === paginationNode.id && e.target === exportNode.id);
    if (connection) {
        console.log("Pagination connects to Export:", connection.sourceHandle.includes('output-2'));
    } else {
