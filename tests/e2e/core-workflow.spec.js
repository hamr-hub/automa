...
test.describe('Automa Core Workflow Features', () => {
  ...
  test('Create, Edit, Save, and Execute a Workflow', async () => {
    ...
    const blockItem = page.locator('.bg-input').filter({ hasText: 'Log Data' }).first();
    ...
    // Verify Block Added
    const logBlock = page.locator('.vue-flow__node-BlockBasic').filter({ hasText: 'Log Data' });
    ...
    await expect(page.locator('#workflow-edit-block p.font-semibold').filter({ hasText: 'Log Data' })).toBeVisible({ timeout: 5000 });
