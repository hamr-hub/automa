...
test.describe('LangGraph and Supabase Integration Test', () => {
  ...
  test('Complete Flow: AI Generation -> UI -> Supabase Sync', async () => {
        ...
        message: {
          ...
          content: JSON.stringify({
            ...
            drawflow: {
              ...
              edges: [
                { source: 'trigger', target: 'log', sourceHandle: 'output', targetHandle: 'input' }
              ]
            }
          })
        },
    ...
    await browserContext.route('**/rest/v1/workflows*', async route => {
        ...
        console.log(`Intercepted Supabase Request: ${method} ${route.request().url()}`);
    ...
    const settingsBtn = page.getByRole('button', { name: /模型配置|Model Config/i });
    ...
    const generateBtn = page.locator('button:has(svg)').filter({ has: page.locator('.ri-send-plane-line') }).or(page.locator('button.absolute.bottom-2.right-2'));
