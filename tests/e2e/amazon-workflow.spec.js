...
test.describe('Automa Extension E2E', () => {
  ...
  test('Import and run Amazon workflow', async () => {
    ...
    const dropdownBtn = page.locator('button:has(.ri-arrow-down-s-line), button:has(svg[data-name="riArrowDownSLine"])').first();
    ...
    // Upload file
    await fileChooser.setFiles(path.join(__dirname, '../fixtures/amazon-workflow.json'));
    console.log('File uploaded');

    // 2. Verify Import
    // Wait for the workflow to appear in the list.
    // The name is "Amazon Laptop Scraper"
    await expect(page.getByText('Amazon Laptop Scraper')).toBeVisible({ timeout: 10000 });
    console.log('Workflow imported successfully');

    // 3. Run Workflow
    // Find the run button for this workflow.
    // The card for the workflow should contain a play button.
    const workflowCard = page.locator('.workflow-card, .ui-card', { hasText: 'Amazon Laptop Scraper' });
