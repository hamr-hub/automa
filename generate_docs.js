    ...
    // Arrow Function / Function Expression assigned to variable: const foo = () => {}
      ...
      methods.push({
        ...
        params: node.init.params
          ? node.init.params.map(extractParamName).join(', ')
          : '',
    ...
    // Property with function: { foo: function() {} } or { foo: () => {} }
      ...
      methods.push({
        ...
        params: node.value.params
          ? node.value.params.map(extractParamName).join(', ')
          : '',
...
console.log(
  `Documentation generation complete. Processed ${processedCount} files with methods.`
);
